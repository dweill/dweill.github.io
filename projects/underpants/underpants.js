...
_.filter = function(arr, funct) {
    var boys = [];
    _.each(arr, function (value, pos, array){
        if(funct(value, pos, array)) {
            boys.push(value);
        }
    }); return boys;
}
...
_.reject = function(arr, test) {
  var rejectArray = _.filter(arr, function(value, pos, array){
      return (!test(value, pos, array));
   });
   return rejectArray;
}
...
_.partition = function(arr, funct) {
   return [_.filter(arr, funct), _.reject(arr, funct)];
...
_.unique = function(arr) {
     //filter the first instance of a number into an array
     var uniqued = [];
     _.each(arr, function(value, pos, array) {
         if(_.indexOf(uniqued, value) === -1) {
             uniqued.push(value);
         }
     });
      return uniqued;
...
_.map = function(coll, test) {
    var treasureMap = [];
    _.each(coll, function(value, pos, array) {
        treasureMap.push(test(value, pos, array));
    });
    return treasureMap;
}
...
_.pluck = function(arro, prop) {
    var feathers =_.map(arro, function(value, pos, array) {
        return value[prop];
    });
    return feathers;
}
...
_.reduce = function(arr, test, seed) {
   ...
   for (; i < arr.length; i++) {
     combined = test(combined, arr[i], i, arr)
   } 
   return combined;
}
